...
class FlspMrpPlanningLine(models.Model):
    ...
    def _flsp_calc_planning(self, standard_lead_time=1, standard_queue_time=1, indirect_lead_time=1, consider_drafts=True, consider_wip=True, consider_forecast=True):
        ...
        for item in open_moves:
            ...
            if new_prod:
                rationale += "</pre>"
                purchase_line = self._include_prod(product, rationale, current_balance, required_by, consider_wip, consumption, False,     standard_lead_time, indirect_lead_time)
                if purchase_line:
                ...
                if forecast_bom:
                    ...
                    for component in forecast_components:
                        ...
                        if not purchase_planning:
                            ...
                            purchase_line = self._include_prod(product, rationale, False, current_date, consider_wip, False, forecasted, standard_lead_time, indirect_lead_time)
                        else:
    ...
    def _include_prod(self, product, rationale, balance, required_by, consider_wip, consumption=False, forecast=False, standard_lead_time=1, indirect_lead_time=1):
