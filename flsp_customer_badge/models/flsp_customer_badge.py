...
class flspcustomerbadge(models.Model):
    ...
    currency_id = fields.Many2one("res.currency", string="Currency", help="Currency of the Annual Program Spend",
                                  required=True)
    annual_program_amount = fields.Monetary(string='Annual Program Spend',
                                            help="Annual Program Spend for the Reward Level", required=True)

    sale_discount = fields.Float(string='Rewards Pricing Discount (%)', digits='Discount', default=0.0,
                                 help="Discount on sale price")
    freight_units_5_to_10_discount = fields.Float(string='Freight Discount with 5-10 units (%)', digits='Discount',
                                                  default=0.0, help="Frieght discount for orders of from 5 to 10 units")
    freight_units_over_10_discount = fields.Float(string='Freight Discount with more than 10 units (%)',
                                                  digits='Discount', default=0.0,
                                                  help="Frieght discount for orders of more than 10 units. 100% Discount means 'Free freight'.")
    ...
    def get_image(self, img_name=False):
        if not img_name:
            return False
        image_path = get_module_resource('flsp_customer_badge', 'static/description', img_name)
