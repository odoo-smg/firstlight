...
class flspmrpeco(models.Model):
    ...
    def _onchange_type_id(self):
        ...
        pi_first_stage = self.env['mrp.eco.stage'].search([('type_ids', 'in', [pi_stage])], limit=1).id
        npi_first_stage = self.env['mrp.eco.stage'].search([('type_ids', 'in', [npi_stage])], limit=1).id
