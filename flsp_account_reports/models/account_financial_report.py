...
class ReportAccountFinancialReport(models.Model):
    _name = "flsp.account.financial.html.report"
    ...
    line_ids = fields.One2many('flsp.account.financial.html.report.line', 'financial_report_id', string='Lines')
    ...
    def _create_action_and_menu(self, parent_id):
        ...
        print('****************** _create_action_and_menu ************************')
        module = self._context.get('install_module', 'flsp_account_reports')
        IMD = self.env['ir.model.data']
        for report in self:
            if not report.generated_menu_id:
                action_vals = {
                    'name': report._get_report_name(),
                    'tag': 'flsp_account_report',
                    'context': {
                        'model': 'flsp.account.financial.html.report',
                        'id': report.id,
                    },
                }
                action_xmlid = "%s.%s" % (module, 'flsp_account_financial_html_report_action_' + str(report.id))
                ...
                menu_xmlid = "%s.%s" % (module, 'flsp_account_financial_html_report_menu_' + str(report.id))
    ...
    def _get_groups(self, domain, group_by):
        ...
        all_report_lines = self.env['flsp.account.financial.html.report.line'].search([('id', 'child_of', self.line_ids.ids)])
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if line_id:
            line_obj = self.env['flsp.account.financial.html.report.line'].search([('id', '=', line_id)])
    ...
    def _get_copied_name(self):
        '''Return a copied name of the flsp.account.financial.html.report record by adding the suffix (copy) at the end
        until the name is unique.

        :return: an unique name for the copied flsp.account.financial.html.report
        '''
        self.ensure_one()
    ...
    def copy(self, default=None):
        ...
        :return: The copied flsp.account.financial.html.report record.
        '''
        self.ensure_one()
        if default is None:
            default = {}
...
class AccountFinancialReportLine(models.Model):
    _name = "flsp.account.financial.html.report.line"
    ...
    financial_report_id = fields.Many2one('flsp.account.financial.html.report', 'Financial Report')
    parent_id = fields.Many2one('flsp.account.financial.html.report.line', string='Parent', ondelete='cascade')
    children_ids = fields.One2many('flsp.account.financial.html.report.line', 'parent_id', string='Children')
    ...
    def _get_copied_code(self):
        ...
        :return: an unique code for the copied flsp.account.financial.html.report.line
        '''
        self.ensure_one()
    ...
    def _get_balance(self, linesDict, currency_table, financial_report, field_names=None):
        ...
        for rec in self:
            res = dict((fn, 0.0) for fn in field_names)
            c = FormulaContext(self.env['flsp.account.financial.html.report.line'],
        ...
        if self.domain and self.groupby and self.show_domain != 'never':
            ...
            for group_index, group in enumerate(groups['ids']):
                ...
                if results_for_group:
                    ...
                    c = FormulaContext(self.env['flsp.account.financial.html.report.line'].with_context(group_domain=self._get_group_domain(group, groups)),
    ...
    def _get_lines(self, financial_report, currency_table, options, linesDicts):
        ...
        for line in self:
            ...
            vals = {
                ...
                'name': line.name,
                'level': line.level,
                'class': 'o_flsp_account_reports_totals_below_sections' if self.env.company.totals_below_sections else '',
            ...
            if line.id in options.get('unfolded_lines', []) or line.show_domain == 'always':
                ...
                for domain_id in domain_ids:
                    ...
                    if line.financial_report_id.name == 'FLSP Aged Receivable':
                ...
                if domain_ids and self.env.company.totals_below_sections:
                    lines.append({
                        ...
                        'name': _('Total') + ' ' + line.name,
                        'level': line.level,
                        'class': 'o_flsp_account_reports_domain_total',
                        'parent_id': line.id,
...
class FormulaLine(object):
    def __init__(self, obj, currency_table, financial_report, type='balance', linesDict=None):
        ...
        if type == 'balance':
            ...
            if obj._name == 'flsp.account.financial.html.report.line':
                ...
                field_names = ['debit', 'credit', 'balance', 'amount_residual']
                res = obj.env['flsp.account.financial.html.report.line']._compute_line(currency_table, financial_report)
    ...
    def _update_translations(self, filter_lang=None):
        ...
        self.env.cr.execute("""
           ...
           SELECT DISTINCT ON (l.code, a.id) l.code, 'model', 'ir.actions.client,name', a.id, t.src, t.value, t.module, t.state
             FROM flsp_account_financial_html_report r
             JOIN ir_act_client a ON (r.name = a.name)
             JOIN ir_translation t ON (t.res_id = r.id AND t.name = 'flsp.account.financial.html.report,name')
