...
class AccountMove(models.Model):
    ...
    def action_open_tax_report(self):
        action = self.env.ref('flsp_account_reports.action_flsp_account_report_gt').read()[0]
    ...
    def refresh_tax_entry(self):
        for move in self.filtered(lambda m: m.is_tax_closing and m.state == 'draft'):
            ...
            ctx = move.env['flsp.account.report']._set_context(options)
            ctx['strict_range'] = True
            move.env['flsp.account.generic.tax.report'].with_context(ctx)._generate_tax_closing_entry(options, move=move, raise_on_empty=True)
    ...
    def _close_tax_entry(self):
        ...
        for move in self:
            ...
            options = move._compute_vat_period_date()
            ctx = self.env['flsp.account.report']._set_context(options)
            ctx['strict_range'] = True
            attachments = self.env['flsp.account.generic.tax.report'].with_context(ctx)._get_vat_report_attachments(options)
            # end activity
            if len(activity):
