...
class AccountReportManager(models.Model):
    _name = 'flsp.account.report.manager'
    ...
    summary = fields.Char()
    footnotes_ids = fields.One2many('flsp.account.report.footnote', 'manager_id')
    company_id = fields.Many2one('res.company')
    financial_report_id = fields.Many2one('flsp.account.financial.html.report')

    def add_footnote(self, text, line):
        return self.env['flsp.account.report.footnote'].create({'line': line, 'text': text, 'manager_id': self.id})

class AccountReportFootnote(models.Model):
    _name = 'flsp.account.report.footnote'
    _description = 'Account Report Footnote'

    text = fields.Char()
    line = fields.Char(index=True)
    manager_id = fields.Many2one('flsp.account.report.manager')

class FlspAccountReport(models.AbstractModel):
    _name = 'flsp.account.report'
    ...
    def _create_hierarchy(self, lines, options):
        ...
        for line in lines + [None]:
            ...
            # And discriminating sections defined by flsp.account.financial.html.report.line
    ...
    def _get_templates(self):
        return {
                'flsp_main_template': 'flsp_account_reports.flsp_main_template',
                'flsp_main_table_header_template': 'flsp_account_reports.flsp_main_table_header',
                'flsp_line_template': 'flsp_account_reports.flsp_line_template',
                'flsp_footnotes_template': 'flsp_account_reports.flsp_footnotes_template',
                'flsp_search_template': 'flsp_account_reports.flsp_search_template',
        }
    ...
    def execute_action(self, options, params=None):
        ...
        if action_type == 'ir.actions.client':
            ...
            if action.tag == 'flsp_account_report':
                options['unfolded_lines'] = []
                options['unfold_all'] = False
                action_read.update({'options': options, 'ignore_session': 'read'})
        if params.get('id'):
            # Add the id of the flsp.account.financial.html.report.line in the action's context
    ...
    def open_tax_report_line(self, options, params=None):
        active_id = int(str(params.get('id')).split('_')[0])
        line = self.env['flsp.account.financial.html.report.line'].browse(active_id)
    ...
    def open_general_ledger(self, options, params=None):
        ...
        ctx = self.env.context.copy()
        ctx.pop('id', '')
        action = self.env.ref('flsp_account_reports.action_flsp_account_report_general_ledger').read()[0]
        account_id = self._get_caret_option_target_id(params.get('id'))
        options['unfolded_lines'] = ['account_%s' % (account_id or '',)]
        options['unfold_all'] = False
        ctx.update({'model': 'flsp.account.general.ledger'})
    ...
    def periodic_tva_entries(self, options):
        ...
        move = self.env['flsp.account.generic.tax.report']._generate_tax_closing_entry(options)
        action = self.env.ref('account.action_move_journal_line').read()[0]
        action = clean_action(action)
        ...
        if options:
            ...
            if params.get('financial_group_line_id'):
                ...
                if not (isinstance(params['financial_group_line_id'], str) and 'hierarchy_' in params['financial_group_line_id']):
                    parent_financial_report_line = self.env['flsp.account.financial.html.report.line'].browse(params['financial_group_line_id'])
    ...
    def get_report_informations(self, options):
        ...
        info = {'options': options,
                ...
                'searchview_html': self.env['ir.ui.view'].render_template(self._get_templates().get('flsp_search_template', 'flsp_account_report.flsp_search_template'), values=searchview_dict),
                }
        return info
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        * if line_id is set, the template used will be the flsp_line_template
        otherwise it uses the flsp_main_template. Reason is for efficiency, when unfolding a line in the report
        ...
        render_template = templates.get('flsp_main_template', 'flsp_account_reports.flsp_main_template')
        if line_id is not None:
            render_template = templates.get('flsp_line_template', 'flsp_account_reports.flsp_line_template')
        ...
        if self.env.context.get('print_mode', False):
            ...
            html = html.replace(b'<div class="js_flsp_account_report_footnotes"></div>', self.get_html_footnotes(footnotes_to_render))
        return html

    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get('flsp_footnotes_template', 'flsp_account_reports.flsp_footnotes_template')
    ...
    def open_report_export_wizard(self, options):
        ...
        opening it. A new flsp_account_report_generation_options key is also added to
        ...
        """
        new_wizard = self.env['flsp_account_reports.export.wizard'].create({'report_model': self._name,'report_id': self.id})
        view_id = self.env.ref('flsp_account_reports.flsp_view_report_export_wizard').id
        new_context = self.env.context.copy()
        new_context['flsp_account_report_generation_options'] = options
        return {
            'type': 'ir.actions.act_window',
            'name': _('Export'),
            'view_mode': 'form',
            'res_model': 'flsp_account_reports.export.wizard',
            'target': 'new',
    ...
    def _get_report_manager(self, options):
        ...
        existing_manager = self.env['flsp.account.report.manager'].search(domain, limit=1)
        if not existing_manager:
            existing_manager = self.env['flsp.account.report.manager'].create({'report_name': self._name, 'company_id': selected_companies and selected_companies[0] or False, 'financial_report_id': self.id if 'id' in dir(self) else False})
    ...
    def print_pdf(self, options):
        return {
                'type': 'ir_actions_flsp_account_report_download',
    ...
    def _replace_class(self):
        ...
        """
        return {b'o_flsp_account_reports_no_print': b'', b'table-responsive': b'', b'<a': b'<span', b'</a>': b'</span>'}
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        body = self.env['ir.ui.view'].render_template(
            "flsp_account_reports.flsp_print_template",
            values=dict(rcontext),
        )
        body_html = self.with_context(print_mode=True).get_html(options)

        body = body.replace(b'<body class="o_flsp_account_reports_body_print">', b'<body class="o_flsp_account_reports_body_print">' + body_html)
        if minimal_layout:
            header = ''
