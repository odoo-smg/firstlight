...
class flsp_account_bank_reconciliation_report(models.AbstractModel):
    _name = 'flsp.account.bank.reconciliation.report'
    ...
    def _add_bank_statement_line(self, currency, line, amount):
        ...
        return {
            ...
            'class': 'o_flsp_account_reports_level3',
        }

    def print_pdf(self, options):
        options['active_id'] = self.env.context.get('active_id')
        return super(flsp_account_bank_reconciliation_report, self).print_pdf(options)

    def print_xlsx(self, options):
        options['active_id'] = self.env.context.get('active_id')
        return super(flsp_account_bank_reconciliation_report, self).print_xlsx(options)
    ...
    def _get_lines(self, options, line_id=None):
        ...
        lines.append(self._add_line(
            ...
            style_class='o_flsp_account_reports_totals_below_sections' if self.env.company.totals_below_sections else '')
        )
