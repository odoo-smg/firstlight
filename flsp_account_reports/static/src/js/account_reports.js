odoo.define('flsp_account_reports.flsp_account_report', function (require) {
'use strict';
...
var accountReportsWidget = AbstractAction.extend({
    ...
    events: {
        'input .o_flsp_account_reports_filter_input': 'filter_accounts',
        'click .o_flsp_account_reports_summary': 'edit_summary',
        'click .js_flsp_account_report_save_summary': 'save_summary',
        'click .o_flsp_account_reports_footnote_icons': 'delete_footnote',
        'click .js_flsp_account_reports_add_footnote': 'add_edit_footnote',
        'click .js_flsp_account_report_foldable': 'fold_unfold',
        'click [action]': 'trigger_action',
        'click .o_flsp_account_reports_load_more span': 'load_more',
        'click .o_flsp_account_reports_table thead th': 'selected_column',
    ...
    init: function(parent, action) {
        ...
        if (this.report_model === undefined) {
            this.report_model = 'flsp.account.report';
        }
    ...
    render: function() {
        ...
        this.update_cp();
        this.$('.js_flsp_account_report_foldable').each(function() {
    ...
    render_template: function() {
        ...
        this.$('.o_content').find('.o_flsp_account_reports_summary_edit').hide();
    ...
    _add_line_classes: function() {
         ...
         * Jquery code:
         *  this.$('.o_flsp_account_report_line').filter(function () {
         *      return $(this).data('unfolded') === 'True';
         *  }).parent().addClass('o_js_flsp_account_report_parent_row_unfolded');
         */
        var el = this.$el[0];
        var report_lines = el.getElementsByClassName('o_flsp_account_report_line');
        for (var l=0; l < report_lines.length; l++) {
            var line = report_lines[l];
            var unfolded = line.dataset.unfolded;
            if (unfolded === 'True') {
                line.parentNode.classList.add('o_js_flsp_account_report_parent_row_unfolded');
            }
        }
        // This selector is not adaptable in pure JS
        this.$('tr[data-parent-id]').addClass('o_js_flsp_account_report_inner_row');
     },
    filter_accounts: function(e) {
        var self = this;
        var query = e.target.value.trim().toLowerCase();
        this.filterOn = false;
        this.$('.o_flsp_account_reports_level2').each(function(index, el) {
            var $accountReportLineFoldable = $(el);
            var line_id = $accountReportLineFoldable.find('.o_flsp_account_report_line').data('id');
            var $childs = self.$('tr[data-parent-id="'+line_id+'"]');
            var lineText = $accountReportLineFoldable.find('.flsp_account_report_line_name')
            ...
            $accountReportLineFoldable.toggleClass('o_flsp_account_reports_filtered_lines', !queryFound);
            $childs.toggleClass('o_flsp_account_reports_filtered_lines', !queryFound);
        ...
        if (this.filterOn) {
            this.$('.o_flsp_account_reports_level1.total').hide();
        }
        else {
            this.$('.o_flsp_account_reports_level1.total').show();
        }
    ...
    _onChangeExpectedDate: function (event) {
        ...
        paymentDatePicker.appendTo($content.find('div.o_flsp_account_reports_payment_date_picker'));
        var save = function () {
            return this._rpc({
                model: 'res.partner',
    ...
    render_searchview_buttons: function() {
        ...
        var $datetimepickers = this.$searchview_buttons.find('.js_flsp_account_reports_datetimepicker');
        ...
        _.each(this.$searchview_buttons.find('.js_flsp_account_report_bool_filter'), function(k) {
            $(k).toggleClass('selected', self.report_options[$(k).data('filter')]);
        });
        _.each(this.$searchview_buttons.find('.js_flsp_account_report_choice_filter'), function(k) {
        ...
        $('.js_flsp_account_report_group_choice_filter', this.$searchview_buttons).each(function (i, el) {
            var $el = $(el);
        ...
        _.each(this.$searchview_buttons.find('.js_flsp_account_reports_one_choice_filter'), function(k) {
            $(k).toggleClass('selected', ''+self.report_options[$(k).data('filter')] === ''+$(k).data('id'));
        });
        // click events
        this.$searchview_buttons.find('.js_flsp_account_report_date_filter').click(function (event) {
        ...
        this.$searchview_buttons.find('.js_flsp_account_report_bool_filter').click(function (event) {
        ...
        $('.js_flsp_account_report_group_choice_filter', this.$searchview_buttons).click(function () {
            var option_value = $(this).data('filter');
        ...
        this.$searchview_buttons.find('.js_flsp_account_report_choice_filter').click(function (event) {
            var option_value = $(this).data('filter');
        ...
        this.$searchview_buttons.find('.js_flsp_account_reports_one_choice_filter').click(function (event) {
            self.report_options[$(this).data('filter')] = $(this).data('id');
            self.reload();
        });
        this.$searchview_buttons.find('.js_flsp_account_report_date_cmp_filter').click(function (event) {
    ...
    edit_summary: function(e) {
        var $textarea = $(e.target).parents('.o_flsp_account_reports_body').find('textarea[name="summary"]');
        var height = Math.max($(e.target).parents('.o_flsp_account_reports_body').find('.o_flsp_account_report_summary').height(), 100); // Compute the height that will be needed
        ...
        $textarea.val(text);
        $(e.target).parents('.o_flsp_account_reports_body').find('.o_flsp_account_reports_summary_edit').show();
        $(e.target).parents('.o_flsp_account_reports_body').find('.o_flsp_account_reports_summary').hide();
        $(e.target).parents('.o_flsp_account_reports_body').find('textarea[name="summary"]').focus();
    },
    save_summary: function(e) {
        var self = this;
        var text = $(e.target).siblings().val().replace(/[ \t]+/g, ' ');
        return this._rpc({
                model: 'flsp.account.report.manager',
                method: 'write',
                args: [this.report_manager_id, {summary: text}],
                context: this.odoo_context,
            })
            .then(function(result){
                self.$el.find('.o_flsp_account_reports_summary_edit').hide();
                self.$el.find('.o_flsp_account_reports_summary').show();
                if (!text) {
                ...
                return $(e.target).parent().siblings('.o_flsp_account_reports_summary').find('> .o_flsp_account_report_summary').html($content);
            });
    },
    render_footnotes: function() {
        var self = this;
        // First assign number based on visible lines
        var $dom_footnotes = self.$el.find('.js_flsp_account_report_line_footnote:not(.folded)');
        $dom_footnotes.html('');
        var number = 1;
        var footnote_to_render = [];
        _.each($dom_footnotes, function(el) {
            if ($(el).parents('.o_flsp_account_reports_filtered_lines').length > 0) {
                return;
            }
            var line_id = $(el).data('id');
            var footnote = _.filter(self.footnotes, function(footnote) {return ''+footnote.line === ''+line_id;});
            if (footnote.length !== 0) {
                $(el).html('<sup><b class="o_flsp_account_reports_footnote_sup"><a href="#footnote'+number+'">'+number+'</a></b></sup>');
        ...
        return this._rpc({
            ...
            .then(function(result){
                return self.$el.find('.js_flsp_account_report_footnotes').html(result);
            });
    },
    add_edit_footnote: function(e) {
        ...
        var save = function() {
            var footnote_text = $('.js_flsp_account_reports_footnote_note').val().replace(/[ \t]+/g, ' ');
            if (!footnote_text && existing_footnote.length === 0) {return;}
            if (existing_footnote.length !== 0) {
                if (!footnote_text) {
                    return self.$el.find('.footnote[data-id="'+existing_footnote[0].id+'"] .o_flsp_account_reports_footnote_icons').click();
                }
                // replace text of existing footnote
                return this._rpc({
                        model: 'flsp.account.report.footnote',
                        method: 'write',
    ...
    fold_unfold: function(e) {
        var self = this;
        if ($(e.target).hasClass('caret') || $(e.target).parents('.o_flsp_account_reports_footnote_sup').length > 0){return;}
        e.stopPropagation();
        e.preventDefault();
    ...
    fold: function(line) {
        ...
        $(line).parent('tr').removeClass('o_js_flsp_account_report_parent_row_unfolded');
        ...
        if ($lines_to_hide.length > 0) {
            ...
            $lines_to_hide.find('.js_flsp_account_report_line_footnote').addClass('folded');
            $lines_to_hide.hide();
    ...
    unfold: function(line) {
        ...
        if ($lines_in_dom.length > 0) {
            $lines_in_dom.find('.js_flsp_account_report_line_footnote').removeClass('folded');
            $lines_in_dom.show();
    ...
    load_more: function (ev) {
        ...
        this._rpc({
            ...
            .then(function (result){
                var $tr = $line.parents('.o_flsp_account_reports_load_more');
                $tr.after(result);
                $tr.remove();
                self._add_line_classes();
            });
    ...
    unfold_all: function(bool) {
        var self = this;
        var lines = this.$el.find('.js_flsp_account_report_foldable');
...
core.action_registry.add('flsp_account_report', accountReportsWidget);
