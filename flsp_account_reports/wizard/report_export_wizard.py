...
class ReportExportWizard(models.Model):
    """ Wizard allowing to export an accounting report in several different formats
    at once, saving them as attachments.
    """
    _name = 'flsp_account_reports.export.wizard'
    _description = "Export wizard for accounting's reports"

    id = fields.Integer(string="Id")
    export_format_ids = fields.Many2many(string="Export to", comodel_name='flsp_account_reports.export.wizard.format', relation="dms_acc_rep_export_wizard_format_rel")
    ...
    def create(self, vals):
        ...
        rslt.doc_name = hasattr(report, 'name') and report.name or report._description # flsp.account.financial.html.report objects have a name field, not flsp.account.report ones
        ...
        for button_dict in report._get_reports_buttons_in_sequence():
            if button_dict.get('file_export_type'):
                self.env['flsp_account_reports.export.wizard.format'].create({
    ...
    def _get_attachments_to_save(self):
        ...
        for format in self.export_format_ids:
            report_action = format.apply_export(self.env.context['flsp_account_report_generation_options'])
...
class ReportExortWizardOption(models.TransientModel):
    _name = 'flsp_account_reports.export.wizard.format'
    _description = "Export format for accounting's reports"

    name = fields.Char(string="Name", required=True)
    fun_to_call = fields.Char(string="Function to Call", required=True)
    export_wizard_id = fields.Many2one(string="Parent Wizard", comodel_name='flsp_account_reports.export.wizard', required=True)
