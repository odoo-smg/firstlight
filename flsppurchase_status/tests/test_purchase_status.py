...
class TestStockQuant(TransactionCase):
    ...
    def test_flsp_status_cancel_before_shipped(self):
        ...
        stock_move =  self.stock_auto.create_stock_move(tc_name, prod_1.id, sp_1.location_id.id, sp_1.location_dest_id.id, sp_1.id, pol_1.product_qty)
        stock_move.purchase_line_id = pol_1
        # stock_move.state = 'waiting'
        ...
        self.assertEquals(po_1.state, 'purchase', 'state=purchase')
        self.assertEquals(po_1.is_shipped, True, 'is_shipped=True')
        # current behavor is that flsp_po_status is not changed when po's state is changed as 'purchase'
        # self.assertEquals(po_1.flsp_po_status, 'confirmed', 'flsp_po_status=confirmed') 
