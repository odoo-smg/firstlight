...
class flsppurchase(models.Model):
    ...
    def _calc_vendor_code(self):
        for line in self:
            ...
            sellers = line.product_id.seller_ids.filtered(lambda s: s.name.active).sorted(lambda s: (s.sequence, -s.min_qty, s.price))
            ...
            for seller in sellers:
                ...
                if float_compare(quantity_uom_seller, seller.min_qty, precision_digits=precision) == -1:
                    continue
                if seller.product_id and seller.product_id != line.product_id:
                    continue
